<div>
        ...
        <div class="container position-relative z-1">
            <div class="row align-items-end gy-4">
                ...
                <div class="col-lg-9 col-md-8 pb-4" data-aos="fade-left">
                    @if(!empty($doctor->hero_tag))
                        <span class="dd-hero-tag">{{ $doctor->hero_tag }}</span>
                    @endif
    ...
    {{-- ═══════════════════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════════════════ --}}
    @php
        $hasStats    = !empty($doctor->stat_experience) || !empty($doctor->stat_patients);
        $hasExpertise = !empty($doctor->expertise);
        $hasChambers  = !empty($doctor->chambers);
    @endphp
    ...
    {{-- ═══════════════════════════════════════════════════════════════════════
         STATS STRIP — only shown when at least one stat is filled
         ═══════════════════════════════════════════════════════════════════════ --}}
    @php
        $stats = [];
        if (!empty($doctor->stat_experience))   $stats[] = ['icon'=>'bi-award-fill',       'val'=>$doctor->stat_experience,   'lbl'=>'Years Experience'];
        if (!empty($doctor->stat_publications))  $stats[] = ['icon'=>'bi-file-earmark-text', 'val'=>$doctor->stat_publications,  'lbl'=>'Publications'];
        if (!empty($doctor->stat_patients))      $stats[] = ['icon'=>'bi-people-fill',       'val'=>$doctor->stat_patients,      'lbl'=>'Patients Treated'];
        if (!empty($doctor->stat_success_rate))  $stats[] = ['icon'=>'bi-heart-pulse-fill',  'val'=>$doctor->stat_success_rate,  'lbl'=>'Success Rate'];
    @endphp
    @if(!empty($stats))
    ...
    {{-- ═══════════════════════════════════════════════════════════════════════
         AREAS OF EXPERTISE — only shown when expertise is set
         ═══════════════════════════════════════════════════════════════════════ --}}
    @if($hasExpertise)
    ...
    {{-- ═══════════════════════════════════════════════════════════════════════
         CHAMBERS — only shown when chambers are set
         ═══════════════════════════════════════════════════════════════════════ --}}
    @if($hasChambers)
    <section id="dd-chambers" class="dd-chambers py-5">
        <div class="container">
            ...
            <div class="row gy-4 justify-content-center">
                @foreach($doctor->chambers as $idx => $ch)
    ...
    {{-- ═══════════════════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════════════════ --}}
    <style>
        .dd-hero { position: relative; background: linear-gradient(135deg, var(--heading-color) 0%, color-mix(in srgb,var(--heading-color),var(--accent-color) 45%) 100%); padding: 5rem 0 0; overflow: hidden; }
        .dd-hero-bg { position: absolute; inset: 0; background-image: radial-gradient(circle at 70% 50%, color-mix(in srgb,var(--accent-color),transparent 70%) 0%, transparent 55%); }
        .dd-hero-photo-wrap { display: inline-block; border-radius: 20px 20px 0 0; overflow: hidden; width: 220px; box-shadow: 0 -6px 40px rgba(0,0,0,.35); border: 4px solid rgba(255,255,255,.2); }
        .dd-hero-photo-wrap img { width: 100%; height: 280px; object-fit: cover; object-position: top; display: block; }
        .dd-hero-tag { display: inline-block; background: var(--accent-color); color: #fff; font-size: .72rem; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; padding: .22rem .8rem; border-radius: 50px; margin-bottom: .7rem; }
        ...
        .dd-position-badge { background: rgba(255,255,255,.12); color: rgba(255,255,255,.9); border: 1px solid rgba(255,255,255,.25); border-radius: 50px; padding: .22rem .9rem; font-size: .78rem; }
        ...
        .dd-social-pill { display: inline-flex; align-items: center; gap: .4rem; background: rgba(255,255,255,.12); border: 1.5px solid rgba(255,255,255,.22); color: #fff; border-radius: 50px; padding: .32rem .9rem .32rem .7rem; font-size: .8rem; font-weight: 600; text-decoration: none; transition: background .25s, border-color .25s, transform .2s, box-shadow .25s; backdrop-filter: blur(4px); }
        .dd-social-pill i { font-size: 1rem; color: var(--sc); transition: transform .25s; }
        .dd-social-pill:hover { background: var(--sc); border-color: var(--sc); color: #fff; transform: translateY(-2px); box-shadow: 0 6px 18px rgba(0,0,0,.28); }
        .dd-social-pill:hover i { color: #fff; transform: scale(1.15); }
        .dd-sticky-nav { background: #fff; border-bottom: 1px solid #eee; position: sticky; top: 0; z-index: 99; box-shadow: 0 2px 12px rgba(0,0,0,.07); }
        .dd-sticky-nav ul { display: flex; list-style: none; margin: 0; padding: 0; gap: .25rem; overflow-x: auto; }
        .dd-sticky-nav ul li a { display: block; padding: .75rem 1.2rem; font-size: .88rem; font-weight: 600; color: var(--default-color); text-decoration: none; white-space: nowrap; border-bottom: 3px solid transparent; transition: .2s; }
        ...
        .dd-label { display: inline-block; background: color-mix(in srgb,var(--accent-color),transparent 85%); color: var(--accent-color); font-size: .72rem; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; padding: .22rem .8rem; border-radius: 50px; margin-bottom: .65rem; }
        ...
        .dd-cta-inner { background: linear-gradient(135deg, var(--heading-color) 0%, color-mix(in srgb,var(--heading-color),var(--accent-color) 45%) 100%); border-radius: 24px; padding: 3rem 2.5rem; box-shadow: 0 12px 48px rgba(0,0,0,.18); }
